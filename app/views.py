...
@app.route("/get/<username>/<repository>/<folder>/<fname>")
@app.route("/get/<username>/<repository>/<folder>")
@app.route("/get/<username>/<repository>")
def slidePath(username,repository,folder = False, fname = False):
    var = Tools.getEnviromentVar()
    user = Git.getGithubUser(var['GITSPEAKER_GH_USERNAME'], var['GITSPEAKER_GH_PASSWORD'])
    repository = Git.getRepository(user, repository)
    dirContent = Git.getDirContentFromRepository(repository, folder)
    for fl in dirContent:
        if  fl.name == fname:
            return render_template('index.html', content = Markup(formatFileContent(
                fl.name, 
                base64.b64decode(fl.content), 
                int(var['GITSPEAKER_GH_MARKDOWNLINESNUMBER']), 
                int(var['GITSPEAKER_GH_CODELINESNUMBER'])
            )));

    return render_template('index.html', content = "Arquivo nao encontrado" )
